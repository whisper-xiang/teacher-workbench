...
| L3 | Card | `--glass` 22–26px 圆角卡片 | 非概览 page-sheet |
| L4 | Chip | 更小玻璃：今日事项行、标签 | 嵌在 L2/L3 内 |
| L5 | Overlay | 搜索、AI、确认框 | z 35–5000，沿用现有宿主 |

**分层规则**

- L2 只用于概览英雄（日期、标题、今日清单）。不要给英雄再套一张大玻璃底板，那会变成「窗户上贴窗户」。
- L3 卡片垂直堆叠，间距 12px；不要用阴影制造假 3D，用叠放与 1px 边。概览不再使用右侧 L3 卡片墙。
- L4 不得再套 L4（事项行里不再放玻璃卡）。
...
```
[ L1 rail 84|228 ]  [ L2 hero max 640px ，右侧留风景 ]
```

侧栏与主栏间隙 14px（`--nav-w + 28px` 含左侧 inset）。
...
### 7.6 Hero title + kicker

- 日期行：真实当天 + 星期 + 开学周次。不要农历散文、节日口号。
- 标题短句（≤ 8 字）：有未完成事项写「今日事项」；全部勾完写「今日事项已完成」；无事项写「今天暂无事项」。
...
- 工作时段整点（8–22 点，每 2 小时）桌面宠物口头提示喝水或活动。不统计次数、不打卡。悬停气泡即可关闭。不要横向小时带、周曲线、右侧统计卡或课程小卡。
...
- 一句人话 + 文字链。「今天暂无事项」时提供去日历 / 进入看板。不要插画吉祥物。
...
2. L1 可收起侧栏  
3. Topbar 欢迎 + 四圆钮  
4. L2 英雄（日期、标题、完成进度、今日未完成清单）  

数据必须真实：`todayIso()`、课程 `weeklyTopics`、任务状态。禁止写死「周一示例」。桌面宠物在工作时段整点提示喝水或活动，悬停气泡可关闭，不统计次数。
...
- 称呼用 `profile.greetingName`（如「王晓媛老师」）。
- 按钮动词：进入看板、全部日程、收起菜单。不要「立即体验」。
- 空状态说明下一步。
- 数字用阿拉伯数字 + 中文单位（第 9 周、已完成 2 / 5）。
